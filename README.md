...
Acceleration benchmarks compare `AUTO`, `LINEAR`, `BVH`, and `GRID` as render profiles over benchmark scenes such as `small-overhead` and `profile-comparison`. They write images, progress metrics, and a manifest, and are excluded from the default test suite because timing results depend on the machine and JVM state.
